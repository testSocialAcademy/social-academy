...
$(function () {
    ...
    var urls = ["https://pixabay.com/api/?key=2980920-46f1aa264b036ffc6e45ebad0&orientation=vertical&q=red+flowers&min_height=500", "https://pixabay.com/api/?key=2980920-46f1aa264b036ffc6e45ebad0&orientation=vertical&q=yellow+flowers&min_height=500"];
    var homeUrl_mu = "index.html";
    var arrImages = void 0;
    var i = 0;
    ...
    //main body
    Promise.all(urls.map(getImages)).then(function (results) {
        var _arrImages;

        arrImages = JSON.parse(results[0]).hits;
        var arrImagesBuff = JSON.parse(results[1]).hits;
        (_arrImages = arrImages).push.apply(_arrImages, _toConsumableArray(arrImagesBuff));
        arrImages.splice(30, 10);
        arrImages.splice(9, 10);
        arrImages.push(arrImages[0]);
        for (var _i4 = 0; _i4 < arrImages.length - 1; _i4++) {
            totalMargin -= arrImages[_i4].webformatWidth;
        }
        var data = { arrImages: arrImages };
        var actualSlide_mu = Handlebars.compile($('#actualSlide_mu').html());
...
(function () {
    ...
    function displayLS_mu() {
        for (var _i5 = 0; _i5 < localStorage.length; _i5++) {
            $("#listLS_mu").append("<li></li>");
            $("#listLS_mu li:last-child").text(localStorage.key(_i5));
            $("#listLS_mu li:last-child").on("click", function () {
                removeLi_mu(this);
            });
        }
